...
export class GithubWatchJob implements IWatchJob {
	...
	private async sendReleaseMessage(
		...
		console.log(
			`[INFO] Sending message for ${repo.name} to ${repo.senderType}`,
		);
		await sender.sendMessage(
			`New release for ${repo.name}: ${release.tag_name}\n${release.html_url}`,
		);
	}
	...
	async run() {
		...
		for (const repo of repositories) {
			const { data: release } = await this.getLastRelease(
				repo.owner,
				repo.name,
			);
			if (release.tag_name === repo.lastVersion) {
				console.log(
					`[SKIP] No new release for ${repo.name} (last: ${repo.lastVersion})`,
				);
				continue;
			}
			console.log(
				`[INFO] New release found for ${repo.name}: ${release.tag_name}`,
			);
